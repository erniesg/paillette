...
def render_methodology():
    out = (
        ...
        '<p class="legend">The first local edge/component cropper is diagnostic only: HCC review showed it can crop to internal details, object-crop clean studio images, and leave decorative frame borders. For final preprocessing, use a two-stage method: conservative border/surround removal first, then SAM3 or another segmentation fallback for framed/matted 2D works where the target is only the artwork image area. The adjacent Aether Modal SAM3 endpoint is a relevant reference: accept a URL or data URL, return a mask and bounding box, then apply the crop locally and keep TIFF output for HCC masters. Local development should run a local service; Modal is only for GPU-backed remote inference when credentials and data-handling rules allow it.</p>'
        '</section><section class="card"><h2>Frame/Border Scan Tool</h2>'
        '<p class="legend">Use <code>node scripts/hcc-frame-crop-tiff.mjs --input-dir=/path/to/images --report=frame-report.jsonl --recursive</code> only as a diagnostic scanner for now. It supports TIFF/JPEG/PNG/WebP input and can write TIFF output, but do not bulk-apply it until failing HCC examples such as <code>2019-00143_01.tif</code>, <code>2019-00143.tif</code>, and <code>2019-00142_03_N(20210917).tif</code> pass visual review.</p>'
