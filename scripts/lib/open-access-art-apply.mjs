...
function artworkStatement(row, generatedAt) {
  ...
  title, artist, year, year_start, year_end, date_text, medium, medium_family,
  classification, subclassification, visual_classification, primary_artist_id, culture, origin,
  ...
  ${sqlValue(row.artist)},
  ${sqlValue(row.year)},
  ${sqlValue(row.year_start)},
  ${sqlValue(row.year_end)},
  ${sqlValue(row.date_text)},
  ${sqlValue(row.medium)},
  ${sqlValue(row.medium_family)},
  ${sqlValue(row.classification)},
  ${sqlValue(row.subclassification)},
  ${sqlValue(row.visual_classification)},
...
ON CONFLICT(id) DO UPDATE SET
  ...
  medium_family = excluded.medium_family,
  classification = excluded.classification,
  subclassification = excluded.subclassification,
  visual_classification = excluded.visual_classification,
...
export function buildOpenAccessVectorLine(
  ...
  return JSON.stringify({
    ...
    metadata: {
      ...
      year: row.year || 0,
      yearStart: row.year_start || row.year || 0,
      yearEnd: row.year_end || row.year || 0,
      classificationNormalized: row.visual_classification || row.classification || '',
      mediumFamily: row.medium_family || '',
