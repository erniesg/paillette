 ...
 * than trust a regex.
 *
 *   node scripts/demo/e2e3/notes.mjs [baseUrl] [runs]
 */
...
const OUT = path.resolve('docs/night/e2e-evidence/fix-iteration-4');
...
for (let i = 1; i <= RUNS; i += 1) {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(SHOTS, `fix4-note-run${i}.png`) });
    run.shot = `docs/night/shots/fix4-note-run${i}.png`;
  } catch (e) {
    run.error = e.message.split('\n').slice(0, 3).join(' | ');
    console.log(`RUN ${i} ERROR: ${run.error}`);
    await page.screenshot({ path: path.join(SHOTS, `fix4-note-run${i}-error.png`) }).catch(() => {});
  }
  runs.push(run);
  await ctx.close();
}

await writeFile(path.join(OUT, 'notes.json'), JSON.stringify(runs, null, 2));
await browser.close();
