...
const shot = async (page, name) => {
  await page.screenshot({ path: path.join(SHOTS, `e2e3-${name}.png`) });
  return `docs/night/shots/e2e3-${name}.png`;
};
...
await writeFile(path.join(OUT, 'loop.json'), JSON.stringify(evidence, null, 2));
await browser.close();
