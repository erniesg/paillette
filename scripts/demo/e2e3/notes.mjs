...
for (let i = 1; i <= RUNS; i += 1) {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(SHOTS, `e2e3-07-note-run${i}.png`) });
    run.shot = `docs/night/shots/e2e3-07-note-run${i}.png`;
  } catch (e) {
    run.error = e.message.split('\n').slice(0, 3).join(' | ');
    console.log(`RUN ${i} ERROR: ${run.error}`);
    await page.screenshot({ path: path.join(SHOTS, `e2e3-07-note-run${i}-error.png`) }).catch(() => {});
  }
  runs.push(run);
  await ctx.close();
}
