...
- Next action: Use `.agent/state/decisions.md` and `docs/nga-launch-readiness.md` to ask for the recommended v1 decision: defer caption/vector provider work, approve only a bounded R2 staging upload after bucket/secret setup, and keep launch issue #21 held until that evidence is attached. Live R2 upload/enqueue now fails unless an approved bucket is passed with `--bucket` or recorded as a nonempty `.agent/storage.yaml` bucket.
