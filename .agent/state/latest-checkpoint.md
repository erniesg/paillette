...
- Added a fixture-backed public search smoke for issue #19 that proves `/api/public-search/open/text` is an API/org alias for upstream `/orgs/open-access-art/search/text` and preserves an NGA result's image, provenance, collection, accession, institution, and source URL metadata.
...
- Remote checkpoint state:
  - Draft PR #24 (`codex/open-access-art-ingest-rucksack-checkpoint`) points remote agents at the hosted unlock portal scaffold and the bounded R2 proof commands.
  - Gate issue comments with marker `rucksack-remote-checkpoint-pr24` link #18, #20, and #21 back to PR #24.
...
2. If browser-based unlock is needed, deploy the unlock portal from PR #24 and set `RUCKSACK_UNLOCK_BASE_URL` in the trusted runtime.
3. If the human approves the storage path on issue #18, create/select the R2 bucket outside git, set the listed R2 secret values, then run only a bounded staging upload first. Prefer `nga-r2-upload-proof` for the strict two-record remote-agent proof unless a larger first batch is explicitly approved.
...
7. Requeue issue #21 only after #18 and #20 are resolved and the awaiting-review evidence issues have been accepted.
8. Review `docs/nga-launch-readiness.md` before approving any bounded staging launch.
