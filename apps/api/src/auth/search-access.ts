...
export class D1SearchAccessRepository implements SearchAccessRepository {
  ...
  async ensureIdentityUser(identity: ExternalIdentity) {
    ...
    try {
      ...
      // The unique (issuer, subject) constraint is the source of truth. A
      // concurrent first sign-in may have won after our initial read.
      const winner = await this.findIdentityUserId(normalized);
