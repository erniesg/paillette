...
export type ZhongZhengArtworkTextureParticle = {
  id: number;
  sourceX: number;
  sourceY: number;
  x: number;
...
export type ZhongZhengArtworkTextureParticleInput = {
  width: number;
  height: number;
  alpha: Uint8ClampedArray | number[];
  materialPixels: Uint8ClampedArray | number[];
  pointer: ZhongZhengPointerState;
  progress: number;
  elapsedMs: number;
  radiusPixels: number;
...
export const buildZhongZhengArtworkTextureParticles = ({
  width,
  height,
  alpha,
  materialPixels,
  ...
  radiusPixels,
  particleCount = 520,
}: ZhongZhengArtworkTextureParticleInput) => {
  if (
    width <= 0 ||
    height <= 0 ||
    alpha.length < width * height ||
    materialPixels.length < width * height * 4 ||
    !pointer.active ||
    progress <= 0 ||
    radiusPixels <= 0
  ) {
  ...
  const pointerY = (pointer.y / 100) * Math.max(1, height - 1);
  const clampedProgress = clampZhongZhengNumber(progress, 0, 1);
  const radius = Math.max(2, radiusPixels);
  const safeParticleCount = Math.max(1, Math.round(particleCount));
  const particles: ZhongZhengArtworkTextureParticle[] = [];
  const morph = getZhongZhengMorphAmount(elapsedMs);
  const columnCount = Math.max(5, Math.min(24, Math.round(radius / 3.7)));
  const trailLength = Math.max(1, Math.ceil(safeParticleCount / columnCount));
  const horizontalSpan = radius * 1.48;
  const verticalSpan = radius * 2.08;
  const rowSpacing = verticalSpan / Math.max(1, trailLength - 1);

  for (let id = 0; id < safeParticleCount; id += 1) {
    const column = id % columnCount;
    const trailIndex = Math.floor(id / columnCount);
    const particleSeed =
      id * 131 +
      Math.round(pointerX) * 17 +
      Math.round(pointerY) * 19;
    const columnUnit =
      columnCount === 1 ? 0.5 : column / Math.max(1, columnCount - 1);
    const columnSeed = getZhongZhengSeededUnit(column * 193 + 17);
    const sourceX =
      pointerX -
      horizontalSpan / 2 +
      columnUnit * horizontalSpan +
      (columnSeed - 0.5) * 2.2;
    const streamSpeed = 0.0042 + columnSeed * 0.0038;
    const fall =
      (elapsedMs * streamSpeed +
        trailIndex * rowSpacing +
        columnSeed * verticalSpan) %
      verticalSpan;
    const sourceY = pointerY - verticalSpan / 2 + fall;
    const sourceAlpha = getZhongZhengMaskAlphaAt(
      alpha,
      width,
      height,
      sourceX,
    ...
    if (sourceAlpha < 44) continue;

    const distance = Math.sqrt(
      (sourceX - pointerX) ** 2 + (sourceY - pointerY) ** 2
    );
    const localInfluence = clampZhongZhengNumber(
      (radius * 1.08 - distance) / Math.max(1, radius * 1.08),
      0,
      1
    );
    if (localInfluence <= 0) continue;

    const densityGate = getZhongZhengSeededUnit(particleSeed + 23);
    if (densityGate > 0.18 + localInfluence * clampedProgress * 0.62) {
      continue;
    }
    ...
    const materialAlpha = Math.min(
      sourceAlpha,
      materialPixels[dataIndex + 3] ?? sourceAlpha
    );
    if (materialAlpha <= 0) continue;

    const horizontalDrift =
      (getZhongZhengSeededUnit(particleSeed + 31) - 0.5) *
      2.1 *
      localInfluence *
      clampedProgress;
    const verticalDrift =
      (0.28 + getZhongZhengSeededUnit(particleSeed + 37) * 1.25) *
      localInfluence *
      clampedProgress;
    let particleX = sourceX + horizontalDrift;
    let particleY = sourceY + verticalDrift;

    if (
      getZhongZhengMaskAlphaAt(alpha, width, height, particleX, particleY) < 38
    ) {
      particleX = sourceX + (particleX - sourceX) * 0.58;
      particleY = sourceY + (particleY - sourceY) * 0.58;
    }
    ...
    const tokenIndex = (column + trailIndex) % 2;
    const token =
      getZhongZhengSeededUnit(particleSeed + 59) < morph
        ? ZHONG_ZHENG_LATIN_MORPH_TOKENS[tokenIndex] ||
          ZHONG_ZHENG_LATIN_MORPH_TOKENS[0]
        : ZHONG_ZHENG_CHINESE_MORPH_TOKENS[tokenIndex] ||
          ZHONG_ZHENG_CHINESE_MORPH_TOKENS[0];
    ...
    particles.push({
      ...
      x: particleX,
      y: particleY,
      red: materialPixels[dataIndex] || 0,
      green: materialPixels[dataIndex + 1] || 0,
      blue: materialPixels[dataIndex + 2] || 0,
      opacity:
        clampedProgress *
        localInfluence *
        (0.42 + getZhongZhengSeededUnit(particleSeed + 47) * 0.42) *
        (materialAlpha / 255),
      size:
        1.55 +
        getZhongZhengSeededUnit(particleSeed + 53) * 2.35 +
        localInfluence * clampedProgress * 0.86,
      rotation: (getZhongZhengSeededUnit(particleSeed + 67) - 0.5) * 0.34,
      token,
      tokenOpacity: 0.42 + localInfluence * clampedProgress * 0.46,
    });
  }
